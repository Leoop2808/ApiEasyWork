...
namespace EasyWorkBusiness.Implementacion
{
    public class AuthenticationBO : IAuthenticationBO
    {
        ...
        public AutenticarTecnicoResponse AutenticarTecnico(AutenticarTecnicoRequest request, string cod_aplicacion, string idLogTexto) 
        {
            try
            {
                var response = new AutenticarTecnicoResponse();

                var resVerUsuTec= _authenticationDO.VerificarUsuarioTecnico(request.usuario, cod_aplicacion, idLogTexto);
                log.Info($"resVerUsuTec --> " + JsonConvert.SerializeObject(resVerUsuTec));
                response.codeRes = resVerUsuTec.codeRes;
                response.messageRes = resVerUsuTec.messageRes;
                response.flgCorreoValidado = true;
                response.flgCelularValidado = resVerUsuTec.flgCelularValidado;
                response.flgMostrarRegistroUsuario = false;

                if (resVerUsuTec.codeRes != HttpStatusCode.OK)
                {
                    return response;
                }

                var resDataPrincipalUsu = _authenticationDO.ObtenerDataPrincipalUsuario(resVerUsuTec.codUsuario, resVerUsuTec.idUsuario, MedioAcceso.COD_AUTH_CORREO_DEFAULT, cod_aplicacion, idLogTexto);
                log.Info($"resDataPrincipalUsu --> " + JsonConvert.SerializeObject(resDataPrincipalUsu));
                if (resDataPrincipalUsu.codeRes != HttpStatusCode.OK)
                {
                    response.codeRes = resDataPrincipalUsu.codeRes;
            ...
            {
                log.Error("Error :" + JsonConvert.SerializeObject(e));
                return new AutenticarTecnicoResponse()
                {
                    codeRes = HttpStatusCode.InternalServerError,
                    messageRes = "Error interno al autenticar técnico."
                };
            }
        }
        public AutenticarTecnicoGoogleResponse AutenticarTecnicoGoogle(AutenticarTecnicoGoogleRequest request, string cod_aplicacion, string idLogTexto) 
        {
            try
            {
                var response = new AutenticarTecnicoGoogleResponse();
                var resDataGoogle = _authenticationDO.ObtenerDataGoogle(request.google_token, cod_aplicacion, idLogTexto);
                log.Info($"resDataGoogle --> " + JsonConvert.SerializeObject(resDataGoogle));
                if (resDataGoogle.codeRes != HttpStatusCode.OK)
                {
                ...
                var resRegDtGoogle = _authenticationDO.RegistrarDatosGoogleTecnico(resDataGoogle.dataGoogle, request.latitud, request.longitud, request.google_token, cod_aplicacion, idLogTexto);
                log.Info($"resRegDtGoogle --> " + JsonConvert.SerializeObject(resRegDtGoogle));
                response.codeRes = resRegDtGoogle.codeRes;
                response.messageRes = resRegDtGoogle.messageRes;
                response.flgCorreoValidado = true;
            ...
            {
                log.Error("Error :" + JsonConvert.SerializeObject(e));
                return new AutenticarTecnicoGoogleResponse()
                {
                    codeRes = HttpStatusCode.InternalServerError,
                    messageRes = "Error interno al autenticar mediante google."
                };
        ...
        public AutenticarTecnicoFacebookResponse AutenticarTecnicoFacebook(AutenticarTecnicoFacebookRequest request, string cod_aplicacion, string idLogTexto) 
        {
            try
            {
                var response = new AutenticarTecnicoFacebookResponse();
                var resDataFacebook = _authenticationDO.ObtenerDataFacebook(request.facebook_token, cod_aplicacion, idLogTexto);
                log.Info($"resDataFacebook --> " + JsonConvert.SerializeObject(resDataFacebook));
                if (resDataFacebook.codeRes != HttpStatusCode.OK)
                {
                ...
                var resRegDtFacebook = _authenticationDO.RegistrarDatosFacebookTecnico(resDataFacebook.dataFacebook, request.latitud, request.longitud, request.facebook_token, cod_aplicacion, idLogTexto);
                log.Info($"resRegDtFacebook --> " + JsonConvert.SerializeObject(resRegDtFacebook));
                response.codeRes = resRegDtFacebook.codeRes;
                response.messageRes = resRegDtFacebook.messageRes;
                response.flgCorreoValidado = true;
            ...
            {
                log.Error("Error :" + JsonConvert.SerializeObject(e));
                return new AutenticarTecnicoFacebookResponse()
                {
                    codeRes = HttpStatusCode.InternalServerError,
                    messageRes = "Error interno al autenticar mediante facebook."
                };
        ...
        public AutenticarTecnicoCelularResponse AutenticarTecnicoCelular(AutenticarTecnicoCelularRequest request, string cod_aplicacion, string idLogTexto)
        {
            try
            {
                var response = new AutenticarTecnicoCelularResponse();

                var resVerCodAut = _authenticationDO.VerificarCodigoAutenticacionTecnico(request.codVerificacion, request.nroCelular, request.latitud, request.longitud, cod_aplicacion, idLogTexto);
                log.Info($"resVerCodAut --> " + JsonConvert.SerializeObject(resVerCodAut));
                response.codeRes = resVerCodAut.codeRes;
                response.messageRes = resVerCodAut.messageRes;
                response.flgCorreoValidado = true;
            ...
            {
                log.Error("Error :" + JsonConvert.SerializeObject(e));
                return new AutenticarTecnicoCelularResponse()
                {
                    codeRes = HttpStatusCode.InternalServerError,
                    messageRes = "Error interno al autenticar mediante celular."
                };
        ...
        public EnviarSmsOrWhatsappResponse EnviarSmsOrWhatsappTecnico(EnviarSmsOrWhatsappRequest request, string cod_aplicacion, string idLogTexto)
        {
            try
            {
                var response = new EnviarSmsOrWhatsappResponse();
                var verifyCode = Helpers.GenerateCode(6);
                var codTipoCodigoVerificacion = (request.tipoEnvio == TipoEnvioCodigoVerificacion.SMS) ? TipoEnvioCodigoVerificacion.COD_SMS : TipoEnvioCodigoVerificacion.COD_WHATSAPP;
                var resRegCodigoVerificacion = _authenticationDO.RegistrarCodigoVerificacionTecnico(verifyCode, String.Empty, request.nroCelular,
                    codTipoCodigoVerificacion, cod_aplicacion, idLogTexto);
                log.Info($"resRegCodigoVerificacion --> " + JsonConvert.SerializeObject(resRegCodigoVerificacion));
                if (resRegCodigoVerificacion.codeRes != HttpStatusCode.Created)
                {
        ...
        public EnviarCodigoVerificacionCorreoResponse EnviarCodigoVerificacionCorreoTecnico(EnviarCodigoVerificacionCorreoRequest request, string cod_aplicacion, string idLogTexto)
        {
            try
            {
                var response = new EnviarCodigoVerificacionCorreoResponse();
                ...
                var resRegCodigoVerificacion = _authenticationDO.RegistrarCodigoVerificacionTecnico(verifyCode, request.correo, String.Empty,
                    TipoEnvioCodigoVerificacion.COD_CORREO, cod_aplicacion, idLogTexto);
                log.Info($"resRegCodigoVerificacion --> " + JsonConvert.SerializeObject(resRegCodigoVerificacion));
                if (resRegCodigoVerificacion.codeRes != HttpStatusCode.Created)
                {
        ...
        public EnviarCorreoCodigoRecuperacionClaveResponse EnviarCorreoCodigoRecuperacionClaveTecnico(EnviarCorreoCodigoRecuperacionClaveRequest request, string cod_aplicacion, string idLogTexto)
        {
            try
            {
                var response = new EnviarCorreoCodigoRecuperacionClaveResponse();
                ...
                var resValExisUsuCorreo = _authenticationDO.ValidarExistenciaUsuarioCorreoTecnico(request.correo, cod_aplicacion, idLogTexto);
                log.Info($"resValExisUsuCorreo --> " + JsonConvert.SerializeObject(resValExisUsuCorreo));
                if (resValExisUsuCorreo.codeRes != HttpStatusCode.OK)
                {
                    response.codeRes = resValExisUsuCorreo.codeRes;
        ...
        public VerificarCodigoVerificacionCorreoResponse VerificarCodigoVerificacionCorreoTecnico(VerificarCodigoVerificacionCorreoRequest request, string cod_aplicacion, string idLogTexto)
        {
            try
            {
                var response = new VerificarCodigoVerificacionCorreoResponse();

                var resRegCodigoVerificacion = _authenticationDO.VerificarCodigoVerificacionTecnico(request.codigoVerificacion, request.correo, String.Empty, false, cod_aplicacion, idLogTexto);
                log.Info($"resRegCodigoVerificacion --> " + JsonConvert.SerializeObject(resRegCodigoVerificacion));
                response.codeRes = resRegCodigoVerificacion.codeRes;
                response.messageRes = resRegCodigoVerificacion.messageRes;
                return response;
        ...
        public VerificarCodigoVerificacionCelularResponse VerificarCodigoVerificacionCelularTecnico(VerificarCodigoVerificacionCelularRequest request, string cod_aplicacion, string idLogTexto)
        {
            try
            {
                var response = new VerificarCodigoVerificacionCelularResponse();

                var resRegCodigoVerificacion = _authenticationDO.VerificarCodigoVerificacionTecnico(request.codigoVerificacion, String.Empty, request.nroCelular, true, cod_aplicacion, idLogTexto);
                log.Info($"resRegCodigoVerificacion --> " + JsonConvert.SerializeObject(resRegCodigoVerificacion));
                response.codeRes = resRegCodigoVerificacion.codeRes;
                response.messageRes = resRegCodigoVerificacion.messageRes;
                return response;
        ...
        public EnviarSmsOrWhatsappAutenticacionResponse EnviarSmsOrWhatsappAutenticacionTecnico(EnviarSmsOrWhatsappAutenticacionRequest request, string cod_aplicacion, string idLogTexto)
        {
            try
            {
                var response = new EnviarSmsOrWhatsappAutenticacionResponse();

                var resValExis = _authenticationDO.ValidarExistenciaUsuarioCelularTecnico(request.nroCelular, cod_aplicacion, idLogTexto);
                log.Info($"resValExis --> " + JsonConvert.SerializeObject(resValExis));
                if (resValExis.codeRes != HttpStatusCode.OK)
                {
                    response.codeRes = resValExis.codeRes;
        ...
        public ObtenerIdentificadorTecnicoResponse ObtenerIdentificadorTecnico(string username, string cod_aplicacion, string idLogTexto) 
        {
            try
            {
                var response = new ObtenerIdentificadorTecnicoResponse();

                var resObtIdTec = _authenticationDO.ObtenerIdentificadorTecnico(username, cod_aplicacion, idLogTexto);
                log.Info($"resObtIdTec --> " + JsonConvert.SerializeObject(resObtIdTec));
                response.codeRes = resObtIdTec.codeRes;
                response.messageRes = resObtIdTec.messageRes;
                return response;
            }
            catch (Exception e)
            {
                ...
                {
                    codeRes = HttpStatusCode.InternalServerError,
                    messageRes = "Error interno al obtener identificador del técnico."
                };
            }
        }
    }
